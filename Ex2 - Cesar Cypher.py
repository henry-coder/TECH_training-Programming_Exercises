...
def clear_string(string: str) -> str:
    string = normalize('NFKD', string).encode(
        'ASCII', 'ignore').decode('ASCII')  # Removendo acentuação
...
print('\033[1;35m', 'Alfabeto Cifrado'.center(52),
      '\033[0;35m'+cipher, cipher.lower()+'\033[m', sep='\n')
cipher += cipher.lower()
...
binary = ''.join(
    b + ' ' for b in map(bin, bytearray(phrase, 'utf-8'))).replace('b', '')
